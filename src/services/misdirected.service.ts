  ...
  return {
    ...
    async matchPayment(
      ...
      if (!deps.invoices || !deps.reconciliation || !deps.audit) {
        throw new Error('Missing dependencies for matchPayment');
      }
      ...
      // Retrieve transactionId from raw_payload or use payment ID as transactionId if not present
      const rawPayload = payment.raw_payload as any;
    ...
    async refundPayment(
      ...
      if (!deps.nombaTransfer || !deps.audit) {
        throw new Error('Missing dependencies for refundPayment');
      }
